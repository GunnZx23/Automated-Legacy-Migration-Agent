...
- Treat clearing a previously valid account selection as a distinct observable transition. Immediately invalidate pending contact work, clear contacts plus loading and empty-result semantics, keep Load disabled, and render nonempty safe selection guidance through `role="alert"`. Clearing state alone is insufficient: the guidance must remain visible after the reset. A new nonblank selection may clear that warning.
- Preserve stale-response safety as an observable outcome: after the account selection changes, an older pending contact success or failure must not overwrite the current contacts, error, or loading state. The asynchronous guard and internal state representation are your implementation choices and will be judged through controller-owned behavior tests.
