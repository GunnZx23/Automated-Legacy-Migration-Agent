...
from legacy_migration_agent.platforms.mulesoft_validation import (
    ...
    MUnitFailureReason,
    MUnitLocalResult,
    MUnitNotApplicableReason,
    MUnitUnavailableReason,
    parse_munit_local_result,
...
@pytest.mark.parametrize("phase", [MUnitExecutionPhase.QUEUED, MUnitExecutionPhase.RUNNING])
def test_explicit_queued_and_running_results_map_nonterminal(phase):
    evidence = parse_munit_local_result(MUnitLocalResult(phase=phase), context=context())

    assert evidence.status is MuleSoftValidationStatus.NONTERMINAL
    assert evidence.validation_terminal is False
    assert evidence.local_process_exited is False
    assert evidence.command_exit_code is None


def test_explicit_not_applicable_result_is_distinct_from_nonterminal():
    evidence = parse_munit_local_result(
        MUnitLocalResult(
            phase=MUnitExecutionPhase.NOT_APPLICABLE,
            not_applicable_reason=MUnitNotApplicableReason.PLATFORM_SLICE_DEFERRED,
        ),
        context=context(),
    )

    assert evidence.status is MuleSoftValidationStatus.NOT_APPLICABLE
    assert evidence.applicable is False
    assert evidence.validation_terminal is True
    assert evidence.not_applicable_reason is MUnitNotApplicableReason.PLATFORM_SLICE_DEFERRED


@pytest.mark.parametrize(
    "reason, exit_code, exited",
    [
        (MUnitUnavailableReason.MAVEN_TOOL_MISSING, None, False),
        (MUnitUnavailableReason.JAVA_RUNTIME_UNAVAILABLE, 1, True),
        (MUnitUnavailableReason.MULE_RUNTIME_UNAVAILABLE, 1, True),
        (MUnitUnavailableReason.DEPENDENCY_RESOLUTION_UNAVAILABLE, 1, True),
        (MUnitUnavailableReason.LICENSE_UNAVAILABLE, 1, True),
        (MUnitUnavailableReason.REPORT_UNAVAILABLE, 1, True),
    ],
)
def test_controlled_environment_and_tool_failures_map_unavailable(reason, exit_code, exited):
    evidence = parse_munit_local_result(
        MUnitLocalResult(
            phase=MUnitExecutionPhase.UNAVAILABLE,
    ...
    assert evidence.status is MuleSoftValidationStatus.UNAVAILABLE
    assert evidence.unavailable_reason is reason
    assert evidence.validation_terminal is False
    assert evidence.local_process_exited is exited


def test_sanitized_local_completed_result_can_pass_or_fail():
    passed = parse_munit_local_result(
        MUnitLocalResult(
    ...
    failed = parse_munit_local_result(
        MUnitLocalResult(
            phase=MUnitExecutionPhase.COMPLETED,
            command_exit_code=1,
            suites=1,
    ...
    assert passed.status is MuleSoftValidationStatus.PASSED
    assert failed.status is MuleSoftValidationStatus.FAILED
    assert failed.failure_reason is MUnitFailureReason.TEST_FAILURES_AND_COMMAND_EXIT
    assert passed.source is failed.source is MuleSoftEvidenceSource.LOCAL_RESULT


@pytest.mark.parametrize(
    "values, match",
    [
        ({"phase": MUnitExecutionPhase.COMPLETED}, "requires an exit code"),
        (
            {
                "phase": MUnitExecutionPhase.COMPLETED,
                "command_exit_code": 0,
        ...
        ({"phase": MUnitExecutionPhase.RUNNING, "command_exit_code": 0}, "cannot claim an exit"),
        ({"phase": MUnitExecutionPhase.NOT_APPLICABLE}, "requires an explicit reason"),
        ({"phase": MUnitExecutionPhase.UNAVAILABLE}, "requires an explicit reason"),
        (
            {
                "phase": MUnitExecutionPhase.UNAVAILABLE,
                "command_exit_code": 0,
                "unavailable_reason": MUnitUnavailableReason.MAVEN_TOOL_MISSING,
            },
            "cannot use command_exit_code 0",
        ),
    ],
)
def test_local_result_contradictions_fail_at_the_contract_boundary(values, match):
    with pytest.raises(ValidationError, match=match):
        MUnitLocalResult(**values)


@pytest.mark.parametrize(
    "field, value", [("command_exit_code", "0"), ("tests", "1"), ("failures", False)]
)
def test_local_result_numeric_fields_do_not_coerce(field, value):
    values = {
        "phase": MUnitExecutionPhase.COMPLETED,
        "command_exit_code": 0,
        "suites": 1,
    ...
    with pytest.raises(ValidationError, match="not coerced"):
        MUnitLocalResult(**values)
